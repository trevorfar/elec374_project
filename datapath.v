...
module datapath(
    input wire clk, clear, input wire [31:0] inport_data_in,
	 ...
	 outport_data_out, pc_data_out, ry_data_out, ram_data_out, mdr_data_out, z_high_data_out, rz_data_out, HI_data_out, z_low_data_out, muxy_data_out, RY_immediate, ir_data_out
		...
		wire hi_out, lo_out, inc_pc, wren, pc_out, zhighout, zlowout, mar_in, mdr_out, pc_in, inport_in, outport_in, mdr_in, ir_in, yin, mdr_read, gra, grb, grc, hi_in, lo_in, cout, inport_out, rz_in, muxy_select, baout, rin, rout, con_in, alu_add;

		 assign hi_out       = control_signals[`HI_OUT];
		 assign lo_out       = control_signals[`LO_OUT];
		 assign inc_pc       = control_signals[`INC_PC];
		 assign wren         = control_signals[`WREN];
		 assign pc_out       = control_signals[`PC_OUT];
		 assign zhighout     = control_signals[`ZHIGHOUT];
		 assign zlowout      = control_signals[`ZLOWOUT];
		 assign mar_in       = control_signals[`MAR_IN];
		 assign mdr_out      = control_signals[`MDR_OUT];
		 assign pc_in        = control_signals[`PC_IN];
		 assign inport_in    = control_signals[`INPORT_IN];
		 assign outport_in   = control_signals[`OUTPORT_IN];
		 assign mdr_in       = control_signals[`MDR_IN];
		 assign ir_in        = control_signals[`IR_IN];
		 assign yin          = control_signals[`YIN];
		 assign mdr_read     = control_signals[`MDR_READ];
		 assign gra          = control_signals[`GRA];
		 assign grb          = control_signals[`GRB];
		 assign grc          = control_signals[`GRC];
		 assign hi_in        = control_signals[`HI_IN];
		 assign lo_in        = control_signals[`LO_IN];
		 assign cout         = control_signals[`COUT];
		 assign inport_out   = control_signals[`INPORT_OUT];
		 assign rz_in        = control_signals[`RZ_IN];
		 assign muxy_select  = control_signals[`MUXY_SELECT];
		 assign baout        = control_signals[`BAOUT];
		 assign rin          = control_signals[`RIN];
		 assign rout         = control_signals[`ROUT];
		 assign con_in       = control_signals[`CON_IN];
		 assign alu_add      = control_signals[`ALU_ADD];
   ...
   wire [31:0] C_sign_extended, r0_data_out_and; 
	...
	encoder_32_to_5 bus_encoder(
	 .encoder_input({{8{1'b0}}, cout, inportout, pc_out, zlowout,
		zhighout, lo_out, hi_out, {reg_out}}),
   ...
   MDR_32_bit MDR(
    ...
    mux_32_bit bus(
	 ...
	 select_and_encode sel_and_enc(.Gra(gra), .Grb(grb), .Grc(control_signals[`GRC]), .Rin(rin), 
	 .Rout(rout), .BAout(baout), .instruction(ir_data_out),
