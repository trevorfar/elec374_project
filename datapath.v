module datapath(
    ...
    output wire [31:0] outport_data_out,
	 output wire [31:0] bus_data, ram_data_out, HI_data_out, LO_data_out, ir_data_out, mdr_data_out, pc_data_out, r3_data_out, r4_data_out, r5_data_out, r6_data_out, r8_data_out, r2_data_out, z_low_data_out, z_high_data_out,
	 ...
	 HI_in, LO_in, Cout, inport_out, rz_in, muxy_select, BAout, Rin, Rout, con_in, R8,
	 output wire con_out,
...
//	 wire [31:0] z_low_data_out;
   ...
   reg_32_bit LO(.clk(clk), .clear(clear), .enable(LO_in), .BusMuxOut(bus_data), .BusMuxIn(LO_data_out));
	...
	pc_32_bit PC(.clk(clk), .clear(clear), .enable(pc_in), .immediate(bus_data), .pc(pc_data_out), .inc_pc(inc_pc), .pc_offset(pc_offset), .set_pc_flag(set_pc_flag));
