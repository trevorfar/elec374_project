...
module datapath(
    input wire clk, clear, input wire [31:0] inport_data_in,
	 ...
	 outport_data_out, pc_data_out, ry_data_out, ram_data_out, mdr_data_out, z_high_data_out, HI_data_out, z_low_data_out, muxy_data_out, RY_immediate, ir_data_out, 
	 output [63:0] rz_data_out
	 );
...
//	end
	...
	control_unit cu(.clk(clk), .clear(clear), .ir_data_out(ir_data_out), .control_signals(control_signals), .opcode(opcode));
   ...
   wire [31:0] C_sign_extended, r0_data_out_and; 
	...
	encoder_32_to_5 bus_encoder(
	 .encoder_input({{8{1'b0}}, control_signals[`COUT], control_signals[`MDR_OUT], control_signals[`INPORT_OUT], control_signals[`PC_OUT], control_signals[`ZLOWOUT],
   ...
   reg_32_bit LO(.clk(clk), .clear(clear), .enable(control_signals[`LO_IN]), .BusMuxOut(bus_data), .BusMuxIn(LO_data_out));
	...
	pc_32_bit PC(.clk(clk), .clear(clear), .enable(control_signals[`PC_IN]), .immediate(bus_data), .pc(pc_data_out), .inc_pc(inc_pc));
